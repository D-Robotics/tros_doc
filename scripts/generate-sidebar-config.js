  ...
  for (const item of items) {
    ...
    if (stat.isDirectory()) {
      ...
      if (frontMatter && (frontMatter.sidebar_versions || frontMatter.sidebar_products)) {
        ...
        console.log(`Found doc scope config: ${relativePath}`);
...
async function main() {
  ...
  for (const key of Object.keys(config)) {
    ...
    if (/^\d+_/.test(key) || /\/\d+_/.test(key)) {
      console.error(`Error: config key "${key}" still contains a numeric prefix`);
      hasErrors = true;
    }
  ...
  if (hasErrors) {
    console.error('\nConfig validation failed. Please check the generation script.\n');
    process.exit(1);
  }
  ...
  console.log(`Total entries found: ${Object.keys(config).length}\n`);
}
