...
This repository contains the source code for the TROS development documentation site, built with Docusaurus. It includes the main Chinese documentation, English translations, site theme customization, document scope filtering (Doc Scope), and an automated release process.
...
Search currently uses a local search plugin:
- Plugin: `@easyops-cn/docusaurus-search-local`
- Config location: `themes` array in `docusaurus.config.js` (`require.resolve("@easyops-cn/docusaurus-search-local")`)
- Search box display: enabled by `type: "search"` in `themeConfig.navbar.items` in `docusaurus.config.js`
...
- English: `http://localhost:3000/en/tros_doc/tros`
- Chinese: `http://localhost:3000/tros_doc/tros`
