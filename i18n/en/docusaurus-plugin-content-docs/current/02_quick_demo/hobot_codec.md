...
The image encoding and decoding functionality is similar to the ROS image_transport package. RDK uses hardware units to accelerate conversion between MJPEG/H264/H265 and BGR8/RGB8/NV12 formats, which can significantly reduce CPU usage while improving format conversion efficiency.
...
3. Open a browser on the PC (Chrome/Firefox/Edge) and enter `http://IP:8000` , where IP is the RDK device IP address. Click Web Display in the upper left to view the real-time JPEG encoded feed.
